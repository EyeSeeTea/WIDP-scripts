...
SELECT query.* FROM (SELECT uid AS event, json.key AS dataElement, jsonb_extract_path_text(json.value, 'value') AS orgUnit
FROM programstageinstance, jsonb_each(programstageinstance.eventdatavalues) AS json
WHERE json.key IN (select uid from dataelement where valuetype='ORGANISATION_UNIT')) AS query
WHERE query.orgUnit IN ('uid1', 'uid2');
